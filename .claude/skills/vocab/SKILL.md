...
背景與踩過的坑見 memory `vocab-prefix-overhaul`。**每一步都要真的做，不可跳。**

---

## 硬規則（違反＝重來）

1. **正確性不信 LLM 記憶**：每字的字首＋字根拆解、典故一律**對 Etymonline 查證**（實際 WebFetch，不憑記憶）；音標一律**查字典（Wiktionary US IPA）再轉 KK**，不 LLM 猜。
...
### 1. 讀照片 → 每組固定字清單
- Aira 會指定要做哪組字首（或哪幾張照片）。**逐張用 Read 開圖**（照片多為旋轉的課本掃描，仔細讀；含手寫紅/藍字＝老師的字根提示與 Etymonline 出處）。
...
### 2. 每組派一個查證 agent（Etymonline 字根＋典故＋US IPA）
- **一組一個 `general-purpose` agent**（`run_in_background: true`，多組可平行）。prompt 用**附錄 E 範本**。
- **兩條非講不可的鐵律寫進 prompt**：①**「不要 spawn 子 agent、自己 sequentially WebFetch」**（否則會 fan-out、只回狀態不回 JSON——坑 2）；②**固定字清單，NEVER 增刪改字，只驗證**。
- agent 回**固定 schema JSON**：`{word, parts:[{text,gloss}], etymology, us_ipa, etymonline_url, notes}`。陷阱字在 `notes` 說明。
- ⚠️ agent 若撞 session 額度或只回狀態列 → 用 `SendMessage(agentId)` 要它「輸出 consolidated JSON、別重抓」，或換一支重跑該組（別原樣重試同一支 2 次以上）。

### 3. 轉 KK ＋ 併照片資料 → 寫該組來源檔
- 把 agent 的 `us_ipa` **轉 KK**（附錄 B 對照）；`phonetic` 存**不含斜線**（卡片自己包 `/…/`）。
- 併：agent 的 `parts`/`etymology` ＋ 我從照片抄的 `zh`/`pos`/`derivatives`/`example` ＋ 我翻的 `example_zh`。
...
- `preview_start tcm-dev`(4330) → 讀 `.env` 取 `SITE_PASSWORD` → 先導到 `/`（landing 公開）再 `POST /api/unlock` 設 cookie → 導 `/vocab`。
- 確認：頁面 `desc` 字數對、字首 chips 全在、`section` 數＝有字的組數、`article` 卡數＝總字數、**0 console error**；抽查幾張卡（含陷阱字）的 parts/字源/衍/例句文字正確；切「原字庫」仍正常（命中率 banner＋懶載）。手機視角 `preview_resize mobile` 不爆版。
- ⚠️ `preview_screenshot` 在本機常 timeout（工具問題非頁面問題）→ 改用 `preview_eval` 讀 DOM 逐項驗證即可，別卡在截圖。
...
## 附錄 E — 查證 agent prompt 範本

> IMPORTANT: Do the WebFetches YOURSELF, sequentially. Do NOT use the Agent/Task tool or spawn sub-agents — YOU output the JSON array. (WebFetch is deferred — run ToolSearch "select:WebFetch" first.)
>
> Verify etymology + US pronunciation for a FIXED list of N words (prefix `<X>` = `<義>`). NEVER invent/add/remove words. Every etymology claim MUST come from a page you actually WebFetch from Etymonline (`https://www.etymonline.com/word/<word>`); if thin on the root, fetch the root's page. Also WebFetch a US pronunciation from Wiktionary (`https://en.wiktionary.org/wiki/<word>`; Cambridge often 403s).
>
