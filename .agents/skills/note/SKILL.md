...
- **更新該科速查總表 `<科目>-summary.mdx`（必做）**：
  - 篇末「展開閱讀」表加一列，**用 markdown 連結 `[標題](/<slug>)`**（`notes.test.ts` 就是比對這個格式，寫成別的形式會紅燈）。
  - 若本篇帶進**只有橫跨多篇才看得出來**的整合（同一套公式的不同形式、同一個量帶動多個趨勢、前後段接得起來的流程），在總表加一節寫它。**沒有就不要硬加**。
  - ⚠️ **總表自成一個體系（Aira 2026-08-05）**：內文**不要寫「這來自第幾篇」「三篇怎麼串」**這種交叉引用，直接把整合後的知識當成自己的內容寫。要連回原篇只在篇末那張「展開閱讀」表。路徑、決策流程這類東西**自己做成獨立元件或圖**（範本＝`TransportRoutes.astro`），別用「篇 → 篇」的列表代替。
- **排序未經 Aira 允許不可擅自決定/更動。**
- 工作摘要以「親愛的 Aira」開頭。

---

## 多篇批次時的司令模式（Aira 2026-09-02，化學六篇實測）

Aira 只給講義目錄、要一次寫多篇時，改走「主 agent 當司令、不逐篇 plan mode」：她的原話是
「很多篇我無法一一審，司令要把關好」「可依你思考整合，哪些放一起講更好理解」。流程：

1. **司令先算範圍**：把講義每一節的關鍵字拿去 grep 三校題庫（`src/data/{CMU,ISU,TCU}.json`），
   以「題數×出現年份」決定寫不寫、合不合併。實例：3.7 冠醚／四級銨 12 年 4 題 → 不寫；
   3.5 聚合物＋3.6 生物分子 → 合併成一篇（縮合聚合講一次套三種生物大分子）。
2. **一篇一個實作 worker（opus）**，prompt 只給該篇特定資訊，共用規則寫成一份簡報檔讓 worker 自己讀
   （範本：scratchpad 的 `chem_worker_brief.md`）。簡報要明講：用題庫決定深度、每個大節附考題 id、
   正確答案只認 `correct_answer`、**不動 notes.ts／summary／不跑 build／不起 dev server**（司令統一做）。
3. **一篇一個 fresh-context 對抗審查（opus）**，用另一份簡報（`chem_reviewer_brief.md`）：
   逐條 PASS/FAIL 附行號與原文、自己 grep 題庫核每節有沒有考、抽 8 個數據上網查、13 題例題逐題核。
4. 修正交回**原作者**（SendMessage 續談，它有整篇 context），複驗交回**原審查者**只核 finding 清單。
   作者有理由不改的條目要列出，複驗者裁定。單行的小修司令自己動手。
5. **司令親自通讀每一篇**（Aira 的要求），做審查者做不到的跨篇對縫：術語同名、數值一致、
   誰負責機制誰負責引用。通讀抓到的實例：例題用 `client:load` 與其他篇不一致、三篇對同一數值的寫法。
6. 每篇定稿就註冊＋checkpoint commit（不 push），全部完成後一次 build＋push。

**實測成本與坑**：實作 worker 約 320k–390k tokens／篇，審查 240k–300k，修正 250k–480k；
**5 個併發撞 session 額度全滅**（12:40 重置），3 到 4 個安全；撞額度後檔案都在，
用 SendMessage 從各自 transcript 續接即可。長度不用行數當標準，用「零考題覆蓋的段落」當刪除標準
（審查者曾以行／題密度推翻司令「太長」的前提）。化學篇**沒有名詞統整表**（生物慣例），
簡報若寫錯 worker 會照做。審查者常抓到的：同物異名未定義、α 氫這類術語同篇兩義、
必背卡與內文數值不一致（卡會被單獨抽進 SRS，最危險）、機制寫反但同篇另一處寫對。
...
- **必背卡＝條列式，不解釋（Aira 2026-08-04 定案）**：每則寫 `"主題：<ul><li>…</li><li>…</li></ul>"`，主題在 18 字內、用**全形冒號**分隔（元件與 `parseCards` 都照這個切）。
  ...
  - **公式寫 `$…$` 走 KaTeX**（`Memorize.astro` 伺服器端渲染），分數用 `\\dfrac` 的完整橫線。⚠️ items 是雙引號字串，**反斜線一律兩個**：`$n＝\\dfrac{m}{M}$`。只寫一個會安靜壞掉（`\t` 變 TAB、其他 `\x` 讓整組 items 變空），`notes.test.ts` 會抓。
  - `<code>` 只留給**非數學的字面值**：單位（`1 atm＝760 mmHg`）、電子組態（`[Ar]4s¹3d⁵`）、符號序列。純文字的下標用 `<sub>`。
  - **太長的流程／順序鏈拆成 `<li>`**，別塞進一個 `<code>`（在 375px 會變成要橫捲的細長條）。
  - 這些 items 會被抽成每日複習的回想卡（`/api/note-review`），**正面只顯示主題與篇名**——所以主題要能單獨當問題問（「節點」「限量試劑」可以，「其他」「補充」不行）。條列越多，卡片能給的漸進提示越有用。
