...
本專案 PDF 改用 **iPad GoodNotes 看（不印紙本）**。**權威規則在 `AGENTS.md`「列印 PDF（GoodNotes 版）」規範**；memory `pdf-print-pagination` 只是機制演化史的背景指標，與 AGENTS.md 衝突時以 AGENTS.md 為準。本檔是**流程＋驗證 checklist**，不重複規則細節，但動手前要一起讀。
...
- **2026-08-05 起筆記不再有考古題區**（`RelatedQuestions` 已刪除），內嵌圖數就等於 `<Figure` 數，不必再加考古題張數。若某篇算出來明顯多出十幾張，那是舊 PDF 沒重產。
