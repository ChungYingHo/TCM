...
9. **結構與元件沿用黃金範本**：開頭一條龍 roadmap → 每概念一個 `##` 大節（一、二、三…）→ 白話解釋在公式前 → 例題用互動 `ExampleQuestion`（選→對→解析）；既有元件（`NoteStats`／`Memorize`／`KeyPoints`／`SolveApproach`／`ExampleQuestion`／`VseprShapes`…）**一律重用勿另造**。
