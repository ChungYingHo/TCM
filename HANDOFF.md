...
> **2026-06-11 排程「有意義化」改版**（Jeremy 要求：每天都要有意義、複習單字要從背過的字抽）。兩階段制：**學習期**（四科筆記首輪未完，前 ~26 個全力日）照舊「每科 1 筆記＋考點對應考題 10 題」；首輪全完成自動切**刷題期**——筆記照常輪替但 UI 標示「第 N 輪・快速複習」，考題改為 `tracks.drill`（全部 6869 題、年份新→舊、四科 round-robin）**循序刷 30 題/天**，視窗由「刷題期完成的 quiz 日數」推進（`deriveCursors` 改為按日期排序重播，學習期的 quiz 日不消耗刷題視窗）。**複習單字**：SRS 到期字優先＋從已背過的字以日期種子隨機抽樣補滿 60/天（`utils/reviewSample.ts`，cap 100）。同日古文 81→108（醫古文／唐詩宋詞補缺）。穩態約 2.5–3.5 小時/天，符合 4 小時預算。schedule.json +147KB（drill 軌道）。同日追加：開課提早一週至 **2026-06-15**（單字進度從零容錯變 6 天緩衝）；**115 年整年保留為 2–4 月全真模擬考、不入刷題軌道**（drill 6,269 題、114→104 新到舊）。
>
