<script lang="ts">
  ...
  // `q`, `options[]` and `steps[]` accept simple HTML plus inline KaTeX such as
  // `$\\ce{NaCl}$`. These strings arrive after MDX compilation, so they need
  // their own rendering pass here.
  let {
    n = 0,
    q = '',
    options = [],
...
<div class="my-3 rounded-box border border-base-300 bg-base-100 print:break-inside-avoid">
  ...
  <div class="flex items-start gap-2 p-3.5">
    ...
    <div class="min-w-0 flex-1">
      <p class="font-medium leading-relaxed">{@html renderRichText(q)}</p>
            ...
            {#each options as opt, i (i)}
              <li>
                ...
                >
                  ...
                  <span class="min-w-0 flex-1">{@html renderRichText(opt)}</span>
          ...
          <ul class="mt-2 grid gap-1 text-sm sm:grid-cols-2">
            {#each options as opt, i (i)}
              <li class="flex gap-1.5">
                <span class="font-semibold text-primary">({letters[i]})</span>
                <span>{@html renderRichText(opt)}</span>
              </li>
            {/each}
          </ul>
        {/if}
    ...
    <div class="animate-fade-in-up border-t border-base-300 p-3.5">
      {#if answer}
        <div class="mb-2 flex flex-wrap items-center gap-2 text-sm">
          ...
          <span class="badge badge-success font-bold">{@html renderRichText(answer)}</span>
        </div>
      {/if}
      {#if steps.length}
        <ol class="flex flex-col gap-2 text-sm leading-relaxed">
          {#each steps as step, i (i)}
            <li class="grid grid-cols-[1.4rem_1fr] items-start gap-1.5">
              ...
              <span class="pt-0.5">{@html renderRichText(step)}</span>
            </li>
          {/each}
        </ol>
      {/if}
  ...
  <div class="hidden print:block p-3.5">
    <p class="font-medium leading-relaxed"><span class="font-bold">例 {n}　</span>{@html renderRichText(q)}</p>
    {#if options.length}
      <div class="mt-2 space-y-1 text-sm">
        {#each options as opt, i (i)}
          <div><span class="font-semibold">({letters[i]})</span> {@html renderRichText(opt)}</div>
        {/each}
      </div>
    {/if}
    <div class="write-area mt-3"></div>
    {#if answer}
      <p class="mt-2 text-sm"><span class="font-bold">答案：</span>{@html renderRichText(answer)}</p>
    {/if}
    {#if steps.length}
      <ol class="mt-1 ml-5 list-decimal space-y-0.5 text-sm">
        {#each steps as s, i (i)}<li>{@html renderRichText(s)}</li>{/each}
      </ol>
    {/if}
  </div>
</div>
