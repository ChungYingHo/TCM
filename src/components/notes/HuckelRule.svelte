<script lang="ts">
  // 休克爾規則：輸入 π 電子數，判斷芳香（4n＋2）、反芳香（4n）或非芳香。
  // 前提是環狀、平面、完全共軛，三者都成立時 π 電子數＝4n＋2 才芳香。
  let pi = $state(6)
  const sPi = $derived(Number.isFinite(pi) && pi > 0 ? Math.round(pi) : 0)
  const result = $derived.by(() => {
    if (sPi % 2 === 1) return { t: '非芳香性（π 電子數需為偶數）', cls: 'badge-ghost', n: '' }
...
<div class="not-prose my-5 rounded-box border border-base-300 bg-base-100 p-4 sm:p-5">
  ...
  <p class="mt-3 text-xs leading-relaxed text-base-content/70">
    先確認結構條件：<b>環狀、平面、環上每個原子都有 p 軌域而完全共軛</b>。三者都成立時，<b>π 電子數 = 4n＋2</b>（2、6、10、14…）為芳香性，<b>π 電子數 = 4n</b>（4、8、12…）為反芳香性，比一般烯烴更不穩定。結構條件有一條不成立，不管電子數多少都是非芳香性。
  </p>
</div>
