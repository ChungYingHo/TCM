...
- **照片 → 字表（2026-09-02 逐張核對完成）**：第二批 26 張照片**全部重新開圖核對**，
  比對「照片上實際印的主字」與字表，**26 張全部一致、172 個主字零漏字**。
  核對方法與踩過的坑記在下面那一節。
- **字表本身**：兩份字表的流水號連續無缺號、無重號（第一批 1–95、第二批 1–172）。
- **字表 → 出貨**：組裝器逐條走字表，只有「已查證」的才輸出，並把**尚未查證的字逐一印出來**，
  結構上不可能靜默漏字。每次跑都會印 `字表 N 字｜已查證 M`。

### 逐張核對照片時的三個坑（下一批照做）

課本紙薄雙面印，每張照片上都有疊影，**一張照片同時會出現三種文字**，只有第一種要數：

| 看到什麼 | 是什麼 | 算不算 |
| :--- | :--- | :--- |
| 濃黑銳利 | 本頁實際印刷 | **算** |
| 鏡像（左右相反） | 同一張紙**背面**那一頁 | 不算 |
| 正向但很淡 | **再下一張紙的正面**透上來 | 不算，最容易誤收 |

另外兩點：

- **主字 vs 次級衍生字**：主字頂格、後接詞性。跟在 `【衍】` 那行下面、與【衍】內容同層縮排的
  是次級衍生字，不是主字（例：`elucidate` 下的 `lucid`、`occur` 下的 `recur`、
  `decadent` 下的 `decay`、`exterminate` 下的 `terminate`）。這些字表都收在 `【衍】` 欄位裡。
- **頁面提早結束是正常的**：課本每個大節（`表示「…」的字根`）都另起新頁，
  所以書頁 82 只有 5 字、書頁 90 只有 1 字、書頁 93 只有 3 字都不是漏抄。
  書頁 90 是最極端的一頁，整頁只有 `relieve` 一個主字。
